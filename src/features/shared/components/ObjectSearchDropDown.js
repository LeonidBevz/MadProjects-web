...
const ObjectSearchDropDown = ({values, displayKey, altFilterKey, chosenOption, setChosenOption, emptyMessage}) => {
    ...
    useEffect(() => {
      ...
      const searchTimer = setTimeout(async() => {
        ...
        setFilteredValues(values.filter(item => 
            (item[displayKey] && item[displayKey].toLowerCase().includes(lowercaseQuery)) || 
            (item[altFilterKey] && item[altFilterKey].toLowerCase().includes(lowercaseQuery))
          ));
    ...
    return (
      <div className="search-dd-container" ref={container}>
        <div className={isDropdownOpen ? "search-dd-button isddactive" : "search-dd-button"} onClick={()=>setIsDropdownOpen(!isDropdownOpen)}>
            <div className="input-text no-trans">{chosenOption ? chosenOption[displayKey] : emptyMessage }</div>
            {isDropdownOpen ? 
          ...
          <div className="search-dropdown">
            ...
            <ul>
              {filteredValues.map((item, index) => (
                <li className={item===chosenOption ? "liactive":""} key={index} onClick={()=>handleLiClicked(item)}>{item[displayKey]}</li>
              ))}
            </ul>
          </div>
        )}
...
export default ObjectSearchDropDown